...
function changeNetworkVersion(versionNum) {
  ...
  for (let edgeId in allEdges) {
    ...
    entitySimThreshold = parseInt(document.getElementById('entity-sim-threshold').value);
    ...
    if (allEdges[edgeId].edge_type === "entity2entity" && allEdges[edgeId].hasOwnProperty('sim') && allEdges[edgeId].hasOwnProperty('news2news_sim') && allEdges[edgeId].news2news_sim > newsSimThreshold && allEdges[edgeId].sim === entitySimThreshold) {
      allEdges[edgeId].hidden = false;
    } else if (allEdges[edgeId].edge_type === "entity2entity" && allEdges[edgeId].hasOwnProperty('sim') && allEdges[edgeId].hasOwnProperty('news2news_sim') ) {
      allEdges[edgeId].hidden = true;
    };
    edgeUpdateArray.push(allEdges[edgeId])
  }
...
function createWindow(params) {
  ...
  if (elementExists) {
    ...
    if (selectedType === 'node') {
      headerColor = nodes.get(selected)['color'];
      ...
      headerColor = edges.get(selected)['color'];
